...
void M24256Write(I2C_HandleTypeDef *hi2c,uint8_t addr,uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i = 0; i < numofpages; i++)
	{
		...
		HAL_I2C_Mem_Write(hi2c, addr, MemAddress, 2, &data[pos], bytesremaining, 1000); // write the data to the EEPROM
...
void M24256WriteNumber(I2C_HandleTypeDef *hi2c,uint8_t addr, uint16_t page, uint16_t offset, float data)
{

	float2Bytes(bytes_temp, data);

	M24256Write(hi2c,addr, page, offset, bytes_temp, 4);
}
...
float M24256ReadNumber(I2C_HandleTypeDef *hi2c,uint8_t addr, uint16_t page, uint16_t offset)
{
	...
	M24256Read(hi2c,addr, page, offset, buffer, 4);

	return (Bytes2float(buffer));
}
/*Write the Double values to the EEPROM
 * @page is the number of the start page. Range from 0 to PAGE_NUM-1
 * @offset is the start byte offset in the page. Range from 0 to PAGE_SIZE-1
 * @data is the  Double value that you want to write
 */
void Double2Bytes(uint8_t *ftoa_bytes_temp, double float_variable)
{
	union
	{
		double a;
...
double Bytes2Double(uint8_t *ftoa_bytes_temp)
{
	union
	{
		double a;
...
void M24256WriteDouble(I2C_HandleTypeDef *hi2c,uint8_t addr, uint16_t page, uint16_t offset, double data)
{
	...
	Double2Bytes(bytes_temp, data);

	M24256Write(hi2c,addr, page, offset, bytes_temp, 8);
}
/* Reads the single Double values from the EEPROM
 * @page is the number of the start page. Range from 0 to PAGE_NUM-1
 * @offset is the start byte offset in the page. Range from 0 to PAGE_SIZE-1
 * @returns the float/integer value
 */
...
double M24256ReadDouble(I2C_HandleTypeDef *hi2c,uint8_t addr, uint16_t page, uint16_t offset)
{
	...
	M24256Read(hi2c,addr, page, offset, buffer, 8);

	return (Bytes2Double(buffer));
}
...
void M24256Read(I2C_HandleTypeDef *hi2c,uint8_t addr, uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i = 0; i < numofpages; i++)
	{
		...
		HAL_I2C_Mem_Read(hi2c, addr, MemAddress, 2, &data[pos], bytesremaining, 1000);
		startPage += 1;
		offset = 0;
...
void M24256PageErase(I2C_HandleTypeDef *hi2c,uint8_t addr, uint16_t page)
{
	...
	// write the data to the EEPROM
	HAL_I2C_Mem_Write(hi2c, addr, MemAddress, 2, data, PAGE_SIZE, 1000);
